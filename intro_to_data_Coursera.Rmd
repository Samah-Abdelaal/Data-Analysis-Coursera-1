...
sfo_feb_flights %>%
  summarise(mean = mean(arr_delay), sd = sd(arr_delay), n = n())
...
nycflights %>%
  group_by(month) %>%
  summarise(mediandd = median(dep_delay)) %>%
  arrange(desc(mediandd))
```
...
# type your code for Question 8 here, and Kn
nycflights <- nycflights %>%
...
nycflights %>%
           group_by(dep_type) %>%
         summarise(perc = sum(arr_type == "on time")/n())
```